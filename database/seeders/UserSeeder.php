...
class UserSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        /** @var Collection<int> */
        $organizationWithAdminIds = Organization::whereRelation('users', 'role_id', 2)
            ->pluck('id');

        // Each seeded organization (except the last one, check the above
        // comment) will have 1 admin and 10 members
        $organizations->each(function (Organization $organization) use ($volume, $organizationWithAdminIds, $users, $factory) {
            // The below condition allows the UserSeeder to be launched multiple
            // times without generating multiple admins, as only 1 admin should
            // exist in each organization
